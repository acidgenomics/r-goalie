#' @importFrom checkmate checkEnvironment
#' @export
checkmate::checkEnvironment

#' @importFrom checkmate check_environment
#' @export
checkmate::check_environment



#' @importFrom checkmate testEnvironment
#' @export
checkmate::testEnvironment

#' @importFrom checkmate test_environment
#' @export
checkmate::test_environment



#' @importFrom checkmate assertEnvironment
#' @export
checkmate::assertEnvironment

#' @importFrom checkmate assert_environment
#' @export
checkmate::assert_environment



#' @importFrom checkmate expect_environment
#' @export
checkmate::expect_environment
